...
import { TrendingUp, TrendingDown, Landmark, ShieldCheck, Calculator, LineChart as ChartIcon } from 'lucide-react';
...
export const Dashboard: React.FC<DashboardProps> = ({ state, onUpdate }) => {
  ...
  return (
    <div className="space-y-6 pb-24">
      <div className="bg-white p-1 rounded-2xl shadow-sm border border-slate-100 flex items-center relative h-12">
        {Object.values(UserType).map((type) => (
          <button
            key={type}
            onClick={() => onUpdate({ userType: type })}
            className={`flex-1 py-2 text-xs font-bold rounded-xl transition-all duration-300 z-10 ${
              state.userType === type ? 'text-white' : 'text-black opacity-40 hover:opacity-100'
            }`}
          >
            {type}
          </button>
        ...
        <div 
          className="absolute h-[calc(100%-8px)] bg-blue-600 rounded-xl transition-all duration-300 ease-out"
          style={{ 
            width: 'calc(33.333% - 4px)',
            left: state.userType === UserType.EMPLOYEE ? '4px' : state.userType === UserType.BUSINESS ? '33.333%' : '66.666%',
            marginLeft: state.userType === UserType.EMPLOYEE ? '0' : state.userType === UserType.BUSINESS ? '2px' : '4px'
          }}
        />
      </div>
