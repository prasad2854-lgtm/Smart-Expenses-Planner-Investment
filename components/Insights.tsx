    ...
    <div className="space-y-8 pb-24 text-black">
      ...
      <div className="bg-gradient-to-br from-blue-600 to-indigo-700 rounded-3xl p-6 text-white shadow-xl shadow-blue-200">
        <div className="flex items-center gap-2 mb-4">
          ...
          <h3 className="font-bold text-sm uppercase tracking-widest">Smart AI Insights</h3>
        </div>
        
        {!aiInsights ? (
          <div className="space-y-4">
            <p className="text-xs text-blue-100 opacity-80 leading-relaxed">
              Based on your {state.userType} profile and historical spending, Gemini can provide tailored investment advice.
            </p>
            <button 
      ...
      <div className="space-y-4">
        ...
        <div className="bg-white p-1 rounded-2xl shadow-sm border border-slate-100 flex items-center relative h-10">
          {(['1M', '3M', 'YTD', 'ALL'] as FilterRange[]).map((range) => (
            <button key={range} onClick={() => setFilterRange(range)} className={`flex-1 z-10 py-1 text-[10px] font-bold transition-colors ${filterRange === range ? 'text-white' : 'text-black opacity-40'}`}>{range}</button>
          ))}
          <div className="absolute h-[calc(100%-8px)] bg-blue-600 rounded-xl shadow-sm transition-all left-1" style={{ width: 'calc(25% - 4px)', transform: `translateX(${filterRange === '1M' ? '0%' : filterRange === '3M' ? '100%' : filterRange === 'YTD' ? '200%' : '300%'})` }} />
        </div>
